...
await interaction.reply({ content: `<@${interaction.member.id}>, ${lang.msg94}`, ephemeral: true }).catch(e => { })
...
await interaction.editReply({ content: `<@${interaction.member.id}>, ${lang.msg95}`, ephemeral: true }).catch(e => { })
}
...
await interaction.reply({ content: `<@${interaction.member.id}>, ${lang.msg97}`, ephemeral: true }).catch(e => { })
...
await interaction.editReply({ content: `<@${interaction.member.id}>, ${lang.msg98}`, ephemeral: true }).catch(e => { })
}
...
if (stp === "add-music") {
    ...
    const playlist = await db.playlist.findOne({ userID: interaction.user.id }).catch(e => { })
    if (!playlist?.playlist?.filter(p => p.name === playlist_name).length > 0) return interaction.editReply({ content: lang.msg96, ephemeral: true }).catch(e => { })
    
    let max_music = client.config.playlistSettings.maxMusic
    if (playlist?.musics?.filter(m => m.playlist_name === playlist_name).length > max_music) return interaction.editReply({ content: lang.msg101.replace("{max_music}", max_music), ephemeral: true }).catch(e => { })
    let res 
    try{
    ...
    return interaction.editReply({ content: lang.msg74, ephemeral: true }).catch(e => { })
    }
    if(!res || !res.length || !res.length > 1) return interaction.editReply({ content: lang.msg74, ephemeral: true }).catch(e => { })
    
    await interaction.editReply({ content: `<@${interaction.member.id}>, ${lang.msg102}`, ephemeral: true }).catch(e => { })
    ...
    await interaction.editReply({ content: `<@${interaction.member.id}>, \`${res[0]?.name}\` ${lang.msg105}`, ephemeral: true }).catch(e => { })
...
await interaction.reply({ content: `<@${interaction.member.id}>, ${lang.msg108}`, ephemeral: true }).catch(e => { })
...
await interaction.editReply({ content: `<@${interaction.member.id}>, ${lang.msg109}`, ephemeral: true }).catch(e => { })
}
