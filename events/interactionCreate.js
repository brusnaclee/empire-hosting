...
Note: YouTube links may not work due to restrictions. Please use alternative platforms such as Spotify, SoundCloud, Deezer, or simply provide the name and artist of the songs.`,
								...
								if (!name)
									return interaction
										.editReply({
											content: `${lang.msg73} <a:Cross:1116983956227772476>`,
											ephemeral: true,
										})
										.catch((e) => {});
								let res;
								...
								if (!res || !res.length || !res.length > 1)
									return interaction
										.editReply({
											content: `${lang.msg74} <a:Cross:1116983956227772476>`,
											ephemeral: true,
										})
										.catch((e) => {});
									...
									.then(async (Message) => {
										...
										collector.on('collect', async (button) => {
											switch (button.customId) {
												case 'cancel':
													{
														...
														embed.setDescription(
															`${lang.msg77} <a:Cross:1116983956227772476>`
														);
														await interaction
															.editReply({
																embeds: [embed],
												...
												default: {
													...
													embed.setThumbnail(
														maxTracks[Number(button.customId) - 1].thumbnail
													);
													embed.setDescription(
										...
										collector.on('end', (msg, reason) => {
											if (reason === 'time') {
												embed.setColor('#FF0000');
												embed.setDescription(
													`${lang.msg80} <a:alert:1116984255755599884>`
												);
												return interaction
													.editReply({
														embeds: [embed],
							...
							// Buat pesan embed
							const embed = new EmbedBuilder()
								.setTitle('Add Button Now Playing')
								.setColor(client.config.embedColor)
								.addFields(
