const language = {
	...
	error1:
		'Токен бота, который вы ввели в свой проект, либо неправильный или НАМЕРЕНИЯ вашего бота ВКЛЮЧЕНЫ!',
	error2:
		'Пожалуйста, установите токен бота в token.js или в файле .env в вашем проекте!',
	loadclientevent: 'Kлиентское Событие Загружено',
	embed1:
		'Вы должны иметь роль <@&{djRole}>(DJ), установленная на этом сервере для использования этой команды. Пользователем без этой роли команда {cmdMAP} нельзя использовать.',
	...
	error4:
		'ПРЕДУПРЕЖДЕНИЕ: Кажется, вы не ввели ссылку Mongodb. Если не введёте действительный ссылку Mongodb в config.js, вы не сможете использовать этот бот.',
	msg13: ` Сейчас воспроизведёт:`,
	msg14:
		'Очередь в данный момент пуста. Пожалуйста, добавьте больше музыки, чтобы насладиться.',
	msg15: 'Я отключился, так как в моем канале никого нет.',
	msg16:
		'Испытываю проблемы с подключением к голосовому каналу. Как будто кто-то меня отключил от канала? Мне очень грустно. 😔',
	...
	msg32: `
		...
		Добро пожаловать на **{client.user.username} Страницу помощи по командам!**
		
		**Instruction**

		> Используйте **/** как глобальный префикс.
		> Используйте **/help [за которым следует ваш вопрос для задания любого вопроса нашему ИИ]** чтобы увидеть четкие детали.
		
		**Music Control Panel Button Instruction**

> ⏪ кнопка используется для перемотки музыки на 15 секунд назад.
> 
> ◀️ кнопка для воспроизведения предыдущей музыки.
> 
> ⏸️ кнопка для приостановки музыки.
> 
> ⏯️ кнопка для возобновления музыки.
> 
> ▶️ кнопка для пропуска текущей музыки.
> 
> ⏩ кнопка используется для перемотки музыки на 15 секунд вперед.
> 
> 🔉 кнопка используется для уменьшения громкости на 15.
> 
> 📃 кнопка для отображения текста песни.
> 
> ⏹️ кнопка для остановки музыки.
> 
> <:autoplay:1230869965435961394> кнопка для включения или выключения автопроигрывания в очереди.
> 
> 🔊 кнопка используется для увеличения громкости на 15.
> 
> 🔀 кнопка для случайного воспроизведения музыки в очереди.
> 
> <:download:1230868574722064446> кнопка для загрузки музыки через gdrive.
> 
> ➕ кнопка для добавления музыки или плейлиста в очередь.
> 
> 💾 кнопка для сохранения музыки в вашем плейлисте.
> 
> 🔁 кнопка для повторения музыки в очереди.
		...
		**Feedback**
		...
		Хотите пообщаться, сообщить о проблеме или оставить отзыв? Присоединяйтесь к [The Great Empire]({config.support}) и общайтесь с другими любителями музыки!
   ...
   > **Выключать Цикл:** Выключает цикл.`,
	...
	msg55: `Не могу подключиться к вашему звуковому каналу. <a:Cross:1116983956227772476>`,
	...
	msg85: `Музыка остановилась. Благодарим вас за использование нашего сервиса. `,
	msg86: 'Обновление',
	msg87: `Текущий громкость: **{queue.volume}** <a:Musicon:1116994369833144350>\n** Чтобы изменить громкость, с \`1\` до \`{maxVol}\` Введите число между.**`,
	...
	msg118:
		'Ваше время истекло для использования этой команды. Вы можете ввести `/playlist list {name}`, чтобы использовать опять эту команду.',
	msg119:
		'Введите **/play playlist <list-name>** команду для слушания этих плейлистов. \nВведите **/playlist list <list-name>** чтобы видеть трек в списке.',
	msg120: 'Пожалуйста, укажите текстовый канал.',
	msg121:
		'<#{channel}> добавлен в список использования команды, теперь команду бота можно использоваться только на каналах в списке.',
	...
	msg126:
		' Здесь список каналов, в которых вы можете использовать команды: {channel_filter}',
	msg127: 'Команда не определена.',
	error7:
		'Пожалуйста пользуйтесь этой командой попозже. Разработникам бота сообщили о возможной ошибке.',
	msg128:
		'Вы меня замолчали, пока музыка воспроизводилась. Вот почему я остановил музыку. Продолжу, если включите мой звук. 😔',
	msg129: 'воспроизводится',
	msg130: 'Пожалуйста, введите действительное число.',
	msg131:
		'Чтобы пользоваться этими командами в списке, вам придётся проголосовать за бота.',
	...
	msg135:
		'Время воспроизведения было установлено на {queue.formattedCurrentTime} успешно.',
	msg136:
		'Автовоспроизведение включено. С этого момента собираюсь включить случайную музыку.',
	msg137: 'Автовоспроизведение выключено.',
	msg138: 'Канал: **{queue?.connection.channel.name}** ',
	msg142: 'Трек {trackName} был удален.',
	msg143: 'Продолжительность',
	msg144: 'Следующая песня',
	msg145: 'Ожидаемое время до воспроизведения',
	msg146: 'Ожидаемое время',
	msg147: 'Позиция в очереди',
	msg148:
		'Я ждал 3 минуты, но, к сожалению, музыка до сих пор не играет. Спасибо за выбор нашего сервиса.',

	msg149:
		'**{trackName}** был перемещен из **очереди {fromOrder}** в **очередь {toOrder}**.',
	msg150: 'Добавить музыку',
	msg151: 'Напишите название музыки.',
};
